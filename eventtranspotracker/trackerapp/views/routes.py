...
class Routes(ViewSet):
    ...
    def create(self, request):
        ''' Handle POST operations and returns JSON serialized route instance'''

        newroute = Route()
        newroute.name = request.data["name"]
        newroute.color = request.data["color"]
        newroute.description = request.data["description"]


        newroute.save()

        serializer = RouteSerializer(
            newroute, context={'request': request})
